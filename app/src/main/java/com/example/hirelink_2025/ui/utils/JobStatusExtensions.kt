package com.example.hirelink_2025.ui.utils
...
fun TextView.applyJobStatus(status: JobStatus, context: Context) {
    ...
    setTextColor(when (status) {
        JobStatus.ACTIVE -> ContextCompat.getColor(context, android.R.color.holo_green_dark)
        JobStatus.CLOSED -> ContextCompat.getColor(context, android.R.color.darker_gray)
        JobStatus.DRAFT -> ContextCompat.getColor(context, android.R.color.holo_orange_dark)
        JobStatus.PENDING_REVIEW -> ContextCompat.getColor(context, android.R.color.holo_blue_dark)
        JobStatus.PAUSED -> ContextCompat.getColor(context, android.R.color.holo_purple)
        JobStatus.FULL -> ContextCompat.getColor(context, android.R.color.holo_red_dark)
        JobStatus.EXPIRED -> ContextCompat.getColor(context, android.R.color.holo_red_dark)
        JobStatus.REJECTED -> ContextCompat.getColor(context, android.R.color.holo_red_dark)
        JobStatus.SUSPENDED -> ContextCompat.getColor(context, android.R.color.black)
    })

    // Fondo básico
    setBackgroundColor(when (status) {
        JobStatus.ACTIVE -> ContextCompat.getColor(context, android.R.color.holo_green_light)
        JobStatus.CLOSED -> ContextCompat.getColor(context, android.R.color.darker_gray)
        JobStatus.DRAFT -> ContextCompat.getColor(context, android.R.color.holo_orange_light)
