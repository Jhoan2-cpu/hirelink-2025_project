...
import com.example.hirelink_2025.databinding.FragmentApplicantAboutBinding
import com.example.hirelink_2025.models.Applicant
import com.example.hirelink_2025.models.ApplicationStatus

class ApplicantAboutFragment : Fragment() {

    private var _binding: FragmentApplicantAboutBinding? = null
    ...
    override fun onCreateView(
        ...
        _binding = FragmentApplicantAboutBinding.inflate(inflater, container, false)
        return binding.root
    }
    ...
    private fun setupAboutInfo() {
        with(binding) {
            // Información básica
            aboutName.text = applicant.name
            aboutProfession.text = applicant.profession
            aboutExperience.text = applicant.experience
            aboutEmail.text = applicant.email
            aboutPhone.text = applicant.phone

            // Carta de presentación
            if (!applicant.coverLetter.isNullOrEmpty()) {
                coverLetterText.text = applicant.coverLetter
            ...
            applicationInfo.text = buildString {
                append("Aplicó ${applicant.applicationDate}")
                append("\nEstado: ${getStatusText(applicant.status)}")
            }
        }
    }
    ...
    companion object {
        ...
        @JvmStatic
        fun newInstance(applicant: Applicant) = ApplicantAboutFragment().apply {
